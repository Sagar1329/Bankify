...
const CategoryBadge=({category}:CategoryBadgeProps) => {
    ...
    const {borderColor,backgroundColor,textColor,chipBackgroundColor}= transactionCategoryStyles[category as keyof typeof transactionCategoryStyles]
    ||transactionCategoryStyles.default
    return (
        <div className={cn('category-badge',borderColor)}>
            <div className={cn('size-2 rounde-full',backgroundColor)}/>
            <p
            className={cn('text-[12px] font-medium',textColor)}
            >{category}</p>
                ...
                {transactions.map((t: Transaction) => {
                    ...
                    return (
                        <TableRow key={t.id}
                         ...
                         !border-b-DEFAULT`}>
                            <TableCell
                                className='max-w-[250px] pl-2 pr-10'>
